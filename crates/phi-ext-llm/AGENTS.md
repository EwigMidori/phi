...
- `GeminiSignaturePolicy` is an explicit host choice: Optional, Required, or PlaceholderForMissing. The latter fills only a missing first-call signature in the outgoing model content with Google's documented placeholder; it does not mutate persisted continuation or override provided signatures. GeminiParts assembles the supported relay header/whole-args pair before sealing; shared runtime never interprets fragments.
